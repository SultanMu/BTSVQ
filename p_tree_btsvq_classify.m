function [child] = p_tree_btsvq_classify (sD, sM, level,filename)
             ...
             try, 
                ...
                if (level(i).child(j).data ~=[]) 
                    ...
                    if i == 1
                        ...
                        sDo = level(i-1).child(ceil(j/2)).sD0;
                        sMo = level(i-1).child(ceil(j/2)).sM0;
                        sMo = som_autolabel(sMo,sDo, 'vote');
                        m0 = sMo.labels(:,1);
                    end
...
%                         class2 = (length(sM0.labels(posterior(:,2)>.8,2))/length (sM0.labels))*100
                    ...
                    if inds11 ~= [] & length(inds11)>1
                        ...
                        plot(1:length(labels1),(sM.codebook(inds11,labels1)),'-c',...
                            1:length(labels1),mean(sM.codebook(inds11,labels1)),'-b', ...
                            (length(labels1)+1):(length(labels1)+length(labels2)),(sM.codebook(inds11,labels2)),'-g', ...
                            (length(labels1)+1):(length(labels1)+length(labels2)),mean(sM.codebook(inds11,labels2)),'-b');
...
%                         hold on;
%                         
%                         % find first ranked gene from ranked_genes11  in sM.codebook and plot its profile
%                         % May be add some dialog
%                         plot (1:length (labels1),sM.codebook(strcmp(sM0.labels(:,1),ranked_genes11(1)),labels1),'-m', ...
%                             (length(labels1)+1):(length(labels1)+length(labels2)),sM.codebook(strcmp(sM0.labels(:,1),ranked_genes12(1)),labels2),'-m');                               
...
%                         end
%                         if ~isempty (inds012)
%                             plot (1:length(labels1),sM.codebook(inds012,labels1),'-k',...
%                                 (length(labels1)+1):(length(labels1)+length(labels2)),sM.codebook(inds012,labels2),'-k');
...
%                         end
                        ...
                        [posterior, class1_labels,class2_labels] = gmm_bays_classify_3c(sM0.labels(:,1),sM.codebook (:,labels1),sM.codebook(:,labels2),sD1,sM11, sD2,sM12);   
                        clf
                        figure(5)
                        plot (posterior(:,1),'.b')
                        hold on
...
%                         figno = char(strcat({'level_'}, {int2str(i)}, {'_child_'},{int2str(j)},{'_profile'}))
                    ...
                    if inds12 ~= [] & length(inds12)>1
                        plot(1:length(labels1),(sM.codebook(inds12,labels1)),'-g',... 
                            1:length(labels1),mean(sM.codebook(inds12,labels1)),'-ob', ...
                            (length(labels1)+1):(length(labels1)+length(labels2)),(sM.codebook(inds12,labels2)),'-c',...
                            (length(labels1)+1):(length(labels1)+length(labels2)),mean(sM.codebook(inds12,labels2)),'-ob');
...
%                         % find first ranked gene from ranked_genes11  in sM.codebook and plot its profile
%                         % May be add some dialog
%                         plot (1:length (labels1),sM.codebook(strcmp(sM0.labels(:,1),ranked_genes12(1)),labels1),'-m', ...
%                             (length(labels1)+1):(length(labels1)+length(labels2)),sM.codebook(strcmp(sM0.labels(:,1),ranked_genes11(1)),labels2),'-m');                               
%                         set(findobj(gca,'Type','line','Color','m'),'Color','m','LineWidth',2);
%                         % legend
...
%                         
                    ...
                    try % avoid any jump, by avoiding mistake in writing, may be empty string                  
                        ...
                        fprintf(fid, '\n Percet Correct= %s', int2str(level(i).child(j).percent));
                        fprintf(fid, '\n');
                        ...
                        fprintf(fid, '\n Percet Correct= %s', int2str(level(i).child(j+1).percent));
                        fprintf(fid, '\n');
                        fclose (fid);
                    catch
