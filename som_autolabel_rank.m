function [sTo_labels,sTo_qerr]  = som_autolabel_rank(sTo, sFrom, mode,inds)
...
if nargin<4 | isempty(ranki_1), ranki_1 = 'no'; end  

% inds
if nargin<5, inds = 1:m; end
...
else [bmus qerr] = som_bmus(sTo,sFrom,1); end

% for each vector in sTo, make a list of all new labels
Labels = cell(size(sTo.labels,1),1);
Qerr = cell(size(sTo.labels,1),1); % Added - inialize Qerr as well
for d=1:length(bmus), 
  m = bmus(d);
  qm = qerr(d);
  if todata
    ...
    for j=1:length(inds), 
      if ~isempty(sFrom.labels{f,inds(j)}), 
        Labels{t}{length(Labels{t})+1} = sFrom.labels{f,inds(j)}; 
          Qerr{t}{length(Labels{t})} = num2str(qerr(d)); % !!!! find out the reson why this is wrking without +1.
      end
    end 
  end
end
% till this point the Qerr as the quantzaion errors of the coresponding BMUS
% the next task would be to just print te Labels and its Qerr, may be just rank the Labels with Qerr
    ...
    for i=1:length(Labels),
        ...
        for j=1:length(Labels{i}),
            ...
            elseif ~any(strcmp(Labels{i}{j},new_labels)), % a new one!
                ...
                new_Qerr{k}= Qerr{i}{j}; % add this new thing
                new_freq(k) = sum(strcmp(new_labels{k},Labels{i}));
                new_freq_qerr(k) = sum(strcmp(new_Qerr{k},Qerr{i}));
        ...
        if length(new_labels) > 0,
            if strcmp(mode,'add1'), 
                % these two line are removed in order to add the q-error with Labels
                %Labels{i} = new_labels;
                %Qerr{i} = new_Qerr; % added
     ...
     % /THIS IS WORKING FINE
                ...
                % annotate the qerr with Labels at each level ---added
                new_qerr =  [];
                for j=1:length(new_labels), 
                    labl = sprintf('%s',new_labels{j});
                    new_labels{j} = labl;
                    labf = sprintf('%s',new_Qerr{j});
                    new_qerr{j} = labf;
                end
                [dummy order_new] = sort (new_qerr);
                Labels{i} = new_labels(order_new);
                Qerr{i} = new_qerr(order_new);
            else
                %sort labels according to frequency
                [dummy order_labels] = sort(1./(1+new_freq));
                [dummy order_qerr] = sort(1./(1+new_freq_qerr));
                new_labels = new_labels(order_labels);
                new_Qerr = new_Qerr (order_labels);
                new_freq = new_freq(order_labels);
                ...
                case 'vote',
                    ...
                    Qerr{i} = {new_Qerr{1}};
                    %Labels{i} = sprintf('%s(%s)',new_labels{1},new_Qerr{1});
...
sTo_labels = som_label(sTo,'add',[1:length(Labels)]',Labels);

if nargout >1 
sTo_qerr = som_label(sTo,'add',[1:length(Qerr)]',Qerr);
%aout = Qerr;
end  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
