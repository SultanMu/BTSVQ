function [child] = p_tree_btsvq_classify_rank_oneup (sD, sM, level,filename)
...
% activate this *****************************
[sM0]= som_autolabel_rank(sM,sD,'add1');
...
%for i = 1 : 8 % 50 levels of Partative tree (may be take it as argument at later stage)     
    try, % exception for the level error
      if isstruct(level(il))
            %fprintf ( 'Level \n',i)
            
            for j = 1:2:2^il
             try, 
                %if isstruct(level(i).child(j))
                if (level(il).child(j).data ~=[]) 
                    fprintf(1,' level / child [%d, %d] \n',il, j);   
                    
                    
                    sD1 = level(il).child(j).sD;
                    sD2 = level(il).child(j+1).sD;
                    sM11 = som_autolabel_rank (level(il).child(j).sM,level(il).child(j).sD,'add1');
                    sM12 = som_autolabel_rank (level(il).child(j+1).sM,level(il).child(j+1).sD,'add1');                                        
                    ...
                    if i == 1
                        labels1 = level(il).child(j).labels; 
                        labels2 = level(il).child(j+1).labels; 
                        m0 = sM0.labels(:,1);
                    else
                    ...
                    level(il).child(j).gene_labels = m11;
                    level(il).child(j+1).gene_labels = m12;
                    level(il).common_gene_labels = c012;
...
%                         class2 = (length(sM0.labels(posterior(:,2)>.8,2))/length (sM0.labels))*100
                    ...
                    if inds11 ~= [] & length(inds11)>1
                        ...
                        title(char(strcat({'level\_'}, {int2str(il)}, {'   child\_'},{int2str(j)})),'Color','b','FontWeight','bold') 
%                         hold on;
%                         
...
%                         end
                        % print to the file
                        figno = char(strcat({'level_'}, {int2str(il)}, {'_child_'},{int2str(j)},{'_profile'}))
                        ...
                        hold on
                        plot (posterior(:,2),'.r')
                        figno = char(strcat({'level_'}, {int2str(il)}, {' child_'},{int2str(j)},{' & '},{int2str(j+1)},{'(prob)'}))
                        print (gcf, '-djpeg', '-r200', figno)
                       ...
                       try 
                           ...
                           level(il).child(j).percent  = (length(common1)/length (ranked_genes11))*100;
                           
                           % similarly
                           [class2_in_sM0] = gene_label_comp_4 (sM0.labels(:,1),class2_labels);
                           [common2] = gene_label_comp_4 (class2_in_sM0,ranked_genes11);
                           level(il).child(j+1).percent = (length(common2)/length(ranked_genes12))*100;
                       catch
                           level(il).child(j).percent = 0;
                           level(il).child(j+1).percent = 0;
                    ...
                    if inds12 ~= [] & length(inds12)>1
                        ...
                        title(char(strcat({'level\_'}, {int2str(il)}, {'   child\_'},{int2str(j+1)})),'Color','b','FontWeight','bold') 
%                         hold on;
%                         
%                         % find first ranked gene from ranked_genes11  in sM0.codebook and plot its profile
%                         % May be add some dialog
...
%                             legend (cated_ranked_genes012,-1);    % make it off
%                         end
                        figno = char(strcat({'level_'}, {int2str(il)}, {'_child_'},{int2str(j+1)},{'_profile'}))
                        print (gcf, '-djpeg', '-r200', figno)
...
%                         end
                    ...
                    level(il).child(j).ranked_gene_labels = cated_ranked_genes11;
                    level(il).child(j+1).ranked_gene_labels =cated_ranked_genes12;                    
           
                    try % avoid any jump, by avoiding mistake in writing, may be empty string                  
                        % the g-tree text file section
                        fid = fopen (filename,'a');
                        fprintf(fid, ' Correct Percentage = %s\n', int2str(level(il).child(j).percent));
                        fprintf (fid,'\n%s\n   >> ',(['level(' int2str(il) ').child(' int2str(j) ')'])); 
                        fprintf(fid, '\n');                               
                        for iii = 1:length(level(il).child(j).ranked_gene_labels)
                            fprintf (fid, '%s\t',char(level(il).child(j).ranked_gene_labels(iii)));
                        end 
                        
                        fprintf(fid, '\n');
                        fprintf(fid, ' Correct Percentage = %s\n', int2str(level(il).child(j+1).percent));
                        fprintf (fid,'\n%s\n   >> ',(['level(' int2str(il) ').child(' int2str(j+1) ')']));    
                        fprintf(fid, '\n');                               
                        for iii = 1:length (level(il).child(j+1).ranked_gene_labels)
                            fprintf (fid, '%s\t',char(level(il).child(j+1).ranked_gene_labels(iii)));
                        end 
                        fprintf(fid, '\n');
                        fclose (fid);
                    catch
